...
def test_singleton_instance(sistema_iot):  # Verifica que solo haya una instancia de SistemaIoT
   
    otra_instancia = SistemaIoT.obtener_instancia()
    assert sistema_iot is otra_instancia

def test_observer_register(sensor, sistema_iot):  # Verifica que el sistema IoT se registra correctamente como observador del sensor
   
    sensor.registrar_observador(sistema_iot)
    assert sistema_iot in sensor._observadores

def test_media_desviacion_strategy():  # Prueba para la estrategia de cálculo de media y desviación estándar
